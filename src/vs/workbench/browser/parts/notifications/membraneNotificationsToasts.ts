...
export class MembraneNotificationsToasts implements INotificationsToastController {
	...
	constructor(
		...
		window.membraneNotificationActionHandler = (response: any) => {
	...
	private forwardNotificationToGaze(item: INotificationViewItem): void {
		...
		window.dispatchEvent(new CustomEvent('membraneNotification', {
			detail: {
				type: 'toast',
	...
	dispose(): void {
		...
		if (window.membraneNotificationActionHandler) {
			delete window.membraneNotificationActionHandler;
		}
	}
}
